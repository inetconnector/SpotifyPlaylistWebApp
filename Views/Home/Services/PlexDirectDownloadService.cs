...
public sealed class PlexDirectDownloadService
{
        ...
        string baseUrl, string plexToken, string playlistKey, CancellationToken ct = default)
    {
        ...
        foreach (var t in md.EnumerateArray())
        {
            ...
            var artist = t.TryGetProperty("grandparentTitle", out var ar) ? ar.GetString() : null;
            var album = t.TryGetProperty("parentTitle", out var al) ? al.GetString() : null;
            var trackNumber = t.TryGetProperty("index", out var idx) && idx.TryGetInt32(out var trackIdx)
                ? trackIdx
                : (int?)null;
            ...
            try
            {
                ...
                size = part.TryGetProperty("size", out var sz) && sz.TryGetInt64(out var s64) ? s64 : null;
            }
            catch
            {
                /* ignore */
            ...
            var baseName = BuildBaseFileName(n, title, artist, album, trackNumber);
            var fileName = Sanitize(string.IsNullOrWhiteSpace(baseName) ? "track" : baseName) + ext;

            var plexUrl =
                $"{baseUrl.TrimEnd('/')}/library/parts/{partId}/file{ext}?download=1&X-Plex-Token={plexToken}";
    ...
    private static string BuildBaseFileName(int n, string title, string? artist, string? album, int? trackNumber)
    {
        var parts = new List<string>();

        parts.Add(n.ToString("D3"));
